...
export default class Overlay extends React.Component {
    ...
    mapLastChampion(data, self, config) {
        ...
        let changes = {};
        let tmpState = self.state; // for reading
        ...
        function changeState(newChanges) {
            ...
            tmpState = { ...tmpState, ...newChanges }; // update tmpState just for reading in subsequent checks
        }
        ...
        if (updateState) self.setState(changes); // ONLY pass explicit changes to merge, avoiding stale state overwrite
    }
    componentDidMount() {
        const { state, config } = this.props;
        if (state && state.blueTeam && state.redTeam) {
            this.mapLastChampion(state, this, config);
        }
        if (state && state.champSelectActive && !this.state.openingAnimationPlayed) {
    ...
    render() {
        ...
        return (<>
            ...
            {this.state.playersInfo.enabled && <div className={cx(this.state.currentPlayerInfoAnimation)} style={{
                ...
                <img src={this.state.currentPlayerInfo.champion?.splashCenteredImg} alt={this.state.currentPlayerInfo.champion?.name || 'Splash'} style={{
                    height: 700,
                    width: 400,
                    minWidth: 100,
                    margin: 'auto',
                ...
                <div style={{
                    ...
                    <img style={{ display: 'block', height: 100, width: 'auto', marginTop: -100, marginLeft: 250, filter: 'drop-shadow(0 0 10px rgba(255, 255, 255, 0.2))' }} src={this.state.currentPlayerInfo.lane} alt="Lane" />
                </div>
                <div style={{
                    marginLeft: 500,
                    color: 'white',
                    ...
                    <img style={{
                        ...
                        border: '1px solid rgba(255, 255, 255, 0.08)',
                        boxShadow: '0 8px 32px rgba(0, 0, 0, 0.4)'
                    }} src={this.state.currentPlayerInfo.playerImg} alt="Player" />
                </div>
            </div>}
        </>
        )
